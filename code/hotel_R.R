...
data.unica <- unique(data.limpia)
data.unica$reservation_date <- data.unica$arrival_date - days(data.unica$lead_time)
data.unica$reservation_date_year <- year(data.unica$reservation_date)
df <- data.unica
...
##################################################################
r_tipo_hotel <- table(df$hotel)
#print(r_tipo_hotel)

#Graficamos el resultado
plot(x = df$hotel,
     xlab="Tipo de Hotel", ylab = "Cantidad de Reservas", col= c("blue","red"),
     ylim=c(0, max(r_tipo_hotel)*1.1),main= "Distribución de Reservas, según tipo de Hotel")

text(
  x = 1:2, y = r_tipo_hotel,
  labels = r_tipo_hotel,  # Texto que se mostrará (cantidad de reservas)
  pos = 3,  # Posición del texto (3 = por encima)
...
reservas_por_anio <- df %>%
  group_by(reservation_date_year) %>%
  summarise(cantidad_reservas = n())  # Contar reservas para cada fecha

barplot(
  reservas_por_anio$cantidad_reservas,
  names.arg = reservas_por_anio$reservation_date_year,  # Etiquetas para el eje X
  col = c("blue", "red","green"),  # Colores para las barras
  xlab = "Año", ylab = "Cantidad de Reservas",
  ylim = c(0, max(reservas_por_anio)*1.2),
  main = "Distribución de cantidad de Reservas por Año"
)
text(x = 1:5, y = reservas_por_anio$cantidad_reservas, 
     labels = reservas_por_anio$cantidad_reservas, 
...
##################################################################
# Convertir arrival_date a formato de fecha
data.unica$arrival_date <- as.Date(data.unica$arrival_date)

# Calcular la fecha de reserva restando el lead_time
data.unica$fecha_reserva <- data.unica$arrival_date - data.unica$lead_time

# Extraer el mes de la fecha de reserva
data.unica$mes_reserva <- format(data.unica$fecha_reserva, "%m")
data.unica$nombre_mes <- format(data.unica$fecha_reserva, "%B")

# Agrupar por mes de reserva y contar el número total de reservas
reservas_por_mes <- data.unica %>%
  group_by(nombre_mes) %>%
  summarise(num_reservas = n())

# Ordenar de manera descendente por el número de reservas
reservas_por_mes <- reservas_por_mes[order(reservas_por_mes$num_reservas, decreasing = TRUE), ]

# Identificar las temporadas
n_meses <- nrow(reservas_por_mes)
cortes <- c(n_meses * 0.3, n_meses * 0.6)

temporadas <- vector(mode = "character", length = n_meses)
temporadas[1:cortes[1]] <- "Alta"
temporadas[(cortes[1] + 1):cortes[2]] <- "Media"
temporadas[(cortes[2] + 1):n_meses] <- "Baja"

# Asignar "Baja" a los meses que no caigan en los rangos predefinidos
temporadas[is.na(temporadas)] <- "Baja"

reservas_por_mes <- cbind(reservas_por_mes, temporadas)
...
library(ggplot2)
ggplot(reservas_por_mes, aes(x = reorder(nombre_mes, -num_reservas), y = num_reservas, fill = temporadas)) +
  geom_bar(stat = "identity") +
  xlab("Mes") +
  ylab("Número de reservas") +
  ggtitle("Reservas por mes y temporada") +
  theme_minimal() +
...
reservas_con_niños <- df$children[df$children>0]
reservas_con_bebes <- df$babies[df$babies>0]

reservas.titulo = c("Con niños", "Con bebes")
reservas.valor = c(length(reservas_con_niños), length(reservas_con_bebes))

reservas <- data.frame(Tipo = reservas.titulo, Cantidad = reservas.valor)
#print(reservas)
ylim_max <- max(reservas.valor)*1.35

barplot(
  reservas$Cantidad,
  names.arg = reservas$Tipo,  # Etiquetas para el eje X
  col = c("blue", "red"),  # Colores para las barras
  xlab = "Tipo de Reserva",
  ylab = "Cantidad de Reservas",
  ylim = c(0, max(reservas.valor)*1.2),
  main = "Distribución de cantidad de Reservas con niños o bebes"
)
text(x = 1:length(reservas$Cantidad), 
     y = reservas$Cantidad, 
     labels = reservas$Cantidad, 
     pos = 3, cex = 0.9, col = "black")
...
park_yes <- length(df$required_car_parking_spaces[df$required_car_parking_spaces > 0])
park_no <- length(df$required_car_parking_spaces[df$required_car_parking_spaces == 0])
park_total <- length(df$required_car_parking_spaces)
park_porcentaje <- c(round((park_yes/park_total)*100,2), 
                     round((park_no/park_total)*100,2))

is_park <- data.frame(Titulo=c("Si","No"), Cantidad=c(park_yes, park_no),
                      Porcentaje= park_porcentaje)


ggplot(is_park, aes(x = "", y = Cantidad, fill = Titulo)) +
  geom_bar(stat = "identity", width = 1) +  # Crear la barra completa
  coord_polar(theta = "y") +  # Convertir la barra en un gráfico de pastel
  geom_text(
    aes(label = paste0(Porcentaje, "%")),  # Mostrar el porcentaje en las etiquetas
    position = position_stack(vjust = 0.5),  # Centrar el texto en cada segmento
    size = 4  # Tamaño del texto
  ) +
  labs(
    title = "Distribución de cantidad de reservas según Estacionamiento Requerido",  # Título del gráfico
    fill = "Requiere Estacionamiento"  # Leyenda para el relleno
  ) + theme_void()

##################################################################
#### vii.¿En qué meses del año se producen más cancelaciones de reservas? ####
##################################################################
# Agrupar por mes y contar las reservas canceladas
cancelaciones_por_mes <- data.unica %>%
  filter(is_canceled == TRUE) %>%
  group_by(arrival_date_month) %>%
  summarise(num_cancelaciones = n()) %>%
  arrange(desc(num_cancelaciones))
...
ggplot(cancelaciones_por_mes, aes(x = arrival_date_month, y = num_cancelaciones)) +
  ...
  ggtitle("Cancelaciones de reservas por mes") +
  theme_minimal()
